...
class window1(window):
    ...
    def sigPopup2ButtonOk(self):
        year = str(self.editorSetH_year.text())
        month =str(self.editorSetH_month.text())
        day = str(self.editorSetH_day.text())
        hour = str(self.editorSetH_h.text())
        minute = str(self.editorSetH_m.text())
        second = str(self.editorSetH_s.text())

        if (utils_date_is_valid(year, month, day, hour, minute,second)):
            cmd = 'sudo date --set "'\
                + month + "/" \
                + day + "/" \
                + year + " " \
        ...
        msg.setWindowTitle("Echec de la mise a jour heure systeme.")
        msg.setText("La date saisie n'est pas correct et/ou doit etre superieur au 01/01/2000.")
